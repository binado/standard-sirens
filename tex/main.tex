...
time delay distribution of binary mergers for modelling the redshift probability
distribution~\cite{Ding_2019, Ye_2021, Leandro_2022}.
...
method to demonstrate its capability to recover an unbiased posterior for $H_0$. They make a series
of simplifying assumptions on $p_{det}$, $p_{pop}$ and $\given{\vec{d}}{\vec{\theta}}$:

\begin{itemize}
	\item To account for selection effects, they neglect the effects of the (sky-dependent) GW detector
	      sensitvity and detector-frame mass. Instead, detection is assumed to happen if the
	      \textit{measured} luminosity distance is smaller than a thresold, $d_L^{\rm{th}}$.
\end{itemize}
