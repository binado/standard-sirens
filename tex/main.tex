...
Another method in the literature consists of assuming a model for the mergers distribution on
redshift based on the star formation rate and time delay distribution of binary mergers (INCLUDE
VALERIO STURANI CITATION).
...
\begin{align}
	\label{eq:stat:explicit-marginalized-omega}
...
In the galaxy catalog method, $\ppop (\vec{\theta}| \Omega, \vec{\lambda})$ is informed from the
data. The inferred redshift distribution of galaxies in the catalog may be sensitive to $\Omega$.
To capture the angular dependence in a computationally tractable way, one can separate the sky into
$\npix$ equally-sized pixels $\Omega_i$. Explicitly marginalizing over the redshift $\vec{\theta}
	\to \{\vec{\theta}, z\}$:

\begin{align}
	\iint \given{\vec{d}, \hat{\Omega}}{\vec{\theta}, \Omega} \ppop (\vec{\theta}| \Omega, \vec{\lambda}) \given{\Omega}{\vec{\lambda}} d\vec{\theta} d\Omega
	\to \\ \iiint \given{\vec{d}, \hat{\Omega}}{\vec{\theta}, z, \Omega} \ppop (\vec{\theta} | z, \Omega, \vec{\lambda}) \given{z}{\Omega, \vec{\lambda}} \given{\Omega}{\vec{\lambda}} d\vec{\theta} d\Omega dz
\end{align}
...
paper, the authors perform a mock data analysis of the dark siren approach using galaxy catalogues
to demonstrate the capability of the method to recover an unbiased posterior for $H_0$. make
simplifying assumptions on $p_{det}$, $p_{pop}$ and $\given{\vec{d}}{\vec{\theta}}$:
