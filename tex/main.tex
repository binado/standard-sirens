...
\begin{align}
	\given{\vec{\lambda}, N }{\vec{d}} & = \given{N}{\vec{\lambda}, \vec{d}}\given{\vec{\lambda}}{\vec{d}} \\
	                                   & = e^{-\ndet}(\ndet)^{\nobs} \pi(N) \pi(\vec{\lambda})
	\prod_{i=1}^{\nobs} \frac{\int \given{\vec{d_i}}{\vec{\theta}} \ppop(\vec{\theta} | \vec{\lambda} ) d\vec{\theta}}{\int \pdet(\vec{\theta}) \ppop(\vec{\theta} | \vec{\lambda} ) d\vec{\theta}}
\end{align}
...
method. The rate parametersa are used in the model to describe how the CBC merger rate evolves with
