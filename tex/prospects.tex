...
\subsection{The population prior: the gravitational-wave case}
...
In this section, we analyse the dependence of $\given{\theta}{\Lambda}$ on redshift. It is useful
to write $\theta = \{\theta', z \}$ and split the population prior as a product of two terms,

\begin{equation}
	\given{\theta}{\Lambda} = \given{\theta'}{z, \Lambda} \given{z}{\Lambda}
\end{equation}

We derive a simplified model to infer the BBH merger rate density ($\rm{Gpc}^{-3} \rm{yr}^{-1})$
over redshift, $\given{z}{\Lambda}$. We first parametrize the merger rate per comoving volume as a
curve following the shape of the star formation rate density~\cite{Madau:2014bja}
