...
redshift. Strictly speaking, $z \notin \theta$, since it is not redshift but luminosity distance
that is inferred in the single-event parameter estimation. In any case, we can marginalize the
posterior probability in Eq.~\eqref{eq:stat:full-hierarchical-posterior} over $z$:

\begin{align}
	\given{\Lambda}{\vec{d}} & = \pi(\Lambda) \alpha(\Lambda)^{-\nobs}
	\prod_{i=1}^{\nobs} \int \given{\vec{d_i}}{\theta} \given{\theta, z}{\Lambda} d\theta dz \\
	                         & =  \pi(\Lambda) \alpha(\Lambda)^{-\nobs}
	\prod_{i=1}^{\nobs} \int \given{\vec{d_i}}{\theta} \given{\theta}{z, \Lambda} \given{z}{\Lambda} d\theta dz
\end{align}

Different methodologies will, in general, differ in the redshift prior $\given{z}{\Lambda}$.

\bv{should we write $\given{\vec{d}}{\theta, z}$ instead of $\given{\vec{d}}{\theta}$? Including $z$ makes sense when differentiating between source-frame and detector-frame masses.
	I have also seen other autors - see Refs.~\onlinecite{Mastrogiovanni:2023emh,gray2023joint} -
	write $\given{\vec{d}}{\theta, z, \Lambda}$, as if the single-event likelihood depended on cosmological parameters as well.}

\bv{
	I do not understand why $\given{\theta}{z, \Lambda}$ does not contain a term $\delta(d_L - d_L(z, \Lambda))$ to constrain the luminosity distance to a specific cosmological model. For instance,

	\begin{align}
		\int \given{\vec{d}_i}{\theta} \given{\theta}{z, \Lambda} \given{z}{\Lambda} d\theta & =
		\int \given{\vec{d}_i}{\theta} \given{\theta'}{d_L, z, \Lambda} \given{d_L}{z, \Lambda} \given{z}{\Lambda} dd_L dz d\theta'                                                                                                \\
		                                                                                     & = \int \given{\vec{d}_i}{\theta} \given{\theta'}{d_L, z, \Lambda} \delta(d_L - d_L(z, \Lambda)) \given{z}{\Lambda} dd_L dz d\theta' \\
		                                                                                     & = \int \given{\vec{d}_i}{\theta} \given{\theta'}{z, \Lambda} \given{z}{\Lambda} dz d\theta'
	\end{align}

	On the other hand, killing the integration over $d_L$ with the delta function will probably mess up
	the likelihood $\given{\vec{d}_i}{\theta}$, as the latter it used to integrate over $\theta$ as a
	Monte Carlo sum per Eq.~\eqref{eq:stat:posterior-monte-carlo-sum}.

	Other authors ignore the $z-$dependence of $\theta$ in their models with the assumption that mass
	and spin distributions do not evolve with redshift~\cite{Mastrogiovanni:2023emh,gray2023joint}.
	Whether that argument is correct is debatable, but they do not mention anything about luminosity
	distance either way. }
...
\begin{align}
	\given{\Lambda}{\vec{d}} & \propto \prod_{i=1}^{\nobs} \int \given{\vec{d_i}}{\theta} \given{\theta}{\Omega, z, m, N_\text{gal}, \Lambda} \given{\Omega, z, m, N_\text{gal}}{\Lambda} d\theta d\Omega  dz dm dN_\text{gal}      \\
	                         ...
	                         & \propto \prod_{i=1}^{\nobs} \int \given{\vec{d_i}}{\theta} \given{\theta}{z, \Lambda} \given{N_\text{gal}}{\Omega, z, m, \Lambda} \given{z}{\Lambda} d\theta d\Omega  dz dm dN_\text{gal}
\end{align}

\bv{I am confused with my own notation. Should I use $z | N_\text{gal}$ or $N_\text{gal} | s$?
